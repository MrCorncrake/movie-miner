package utils;

public abstract class DiagramGlobals {
    public static final String EDITING_TOOL_VERSION = "1.0";
    public static final String TOOL_ID = "JaWE";
    public static final String DEFAULT_BORDER_COLOUR = "0,0,0";
    public static final String DEFAULT_TRANSITION_COLOUR = "100,100,100";
    public static final String POOL_DEFAULT_FILL_COLOUR = "255,255,215";
    public static final String POOL_LANE_FILL_COLOUR = "220,220,220";
    public static final String ACTIVITY_FILL_COLOUR = "102,204,51";
    public static final String TRANSITION_STYLE = "NO_ROUTING_SPLINE";
    public static final Integer ACTIVITY_Y_BASE = 55;
    public static final Integer ACTIVITY_X_BASE = 70;
    public static final Integer ACTIVITY_SPACING = 110;
    public static final Integer START_ACTIVITY_SIZE = 31;
    public static final Integer START_ACTIVITY_X_OFFSET = 0;
    public static final Integer START_ACTIVITY_Y_OFFSET = -3;
    public static final Integer END_ACTIVITY_X_OFFSET = 20;
    public static final Integer END_ACTIVITY_Y_OFFSET = 0;
    public static final Integer END_ACTIVITY_SIZE = 31;
    public static final Integer ACTIVITY_WIDTH = 90;
    public static final Integer ACTIVITY_HEIGHT = 60;
}
